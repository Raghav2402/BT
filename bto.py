...
async def call_llm(Prompt):
    ...
    gemini_api_key = "AIzaSyCtDTVY-51eLFblC0P_zN3Ja5sg6oJ8FQM"
    gemini_direct_url = (
...
async def propose_transmission(num_transrack,mw):
    """
    Propose transmission
    """
    
    if mw.lower() == "true":
        return f"{num_transrack} transmission rack are awailable with MicroWave dish"
    else:
        return f"{num_transrack} transmission rack are awailable with no MicroWave dish"
...
async def propose_cabinet(site_type,dep_env,fencing,exist_cabin,radio,avail_spaces,shared_cabin,End_of_EE,End_of_3UK):
    ...
    prompt_cabin = f"""
                ...
                1. AIRI
                - Size(HeightxWidthxDepth): 2000 × 600 × 600 mm
                - Max ERS: 6
                - No PSU (uses DCDU)
                ...
                - Size(HeightxWidthxDepth): 2000 × 1500 × 700 mm
                - Max ERS: 15
                - Radios: same as AIRI
                - Fencing required

                3. S-AIRI (Slimline AIRI – used when width < 1500 mm)
                - Size(HeightxWidthxDepth): 2000 × 1200 × 700 mm
                - Max ERS: 15
                - Radios: same as AIRI
                - Fencing required
                ...
                5. AIRO
                - Size(HeightxWidthxDepth): 2100 × 750 × 600 mm
                - Max ERS: 3 + BBU + PSU
                - Radios: 4490, 4480, 4419, 2460, 4486, 2212, 2260, 2262   (NO 8863)
                - Fencing required

                6. D-AIRO (Double AIRO)
                - Size(HeightxWidthxDepth): 2000 × 1800 × 700 mm
                - Max ERS: 12
                - Radios: same as AIRO
                - Fencing required
                ...
                9. Wiltshire + E6130
                - (Wiltshire Size(HeightxWidthxDepth): 1650 × 2000 × 755 mm) + (E6130 Size(HeightxWidthxDepth): 130 × 720 × 760 mm)
                - max 9 ERS
                - radios: 4490,4480,4486
                - No fencing required

                10. Porter
                - Size(HeightxWidthxDepth): 1452 × 1450 × 650 mm
                - max 6 ERS
                - radios: 4419,2260,2262
                - No fencing required

                11. Weston
                - Size(HeightxWidthxDepth): 1260 × 770 × 700 mm
                - max 3 ERS
                - radios: 2260
                - No fencing required

                12. E6130 standalone
                - Size(HeightxWidthxDepth): 130 × 720 × 760 mm
                ...
                **Most Important**: Use Streetwork family cabinets when sitetype streetwork and similarly for other sitetypes and ignore other parameters, And if the dep_env is Indoor then always consider fencing as yes even if the parameter above says no.
                ...
                2. If fencing is yes → Use any AIRI or AIRO cabinet that fits the requirement based on indoor or outdoor
                ...
                However, if you are not able to propose a single cabinet then in your Output, proposed cabinet will be the one which support more technologies based on ERS and in reasoning must write that either we have to propose both the choosed cabinets(write cabinet names here) or remove the specific radio(write radio name here) for proposing 1 cabinet.
...
{{
  ...
  "reason": "short reason for radio choice"
    ...
    try:
        ...
        required_radio_keys = {"radio", "reason"}
    ...
    with time_it(f"Request ID: {req_id} Total /bt_res_mant request procssing"):
        try:
            ...
            if exist_name_1 != "":
                ...
                gemini_json = await call_llm(get_antenna_details)
            ...
            radio_json = await propose_radio(req)
            ...
            transmission = await propose_transmission(num_transrack,mw)
